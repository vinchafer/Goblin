...
| `worker: fail`, `10085 R2 bucket '…' not found` | Der Bucket liegt in einer **Jurisdiktion** (bei uns: EU) und die Bindung hat sie nicht genannt. Ein EU-Bucket liegt in einem eigenen Namensraum — aus dem Standard-Namensraum gesehen ist er wirklich „nicht da". **Nicht** das Token: wer eine Antwort über einen Bucket bekommt, ist längst autorisiert | Railway → Goblin-API → Variables → `CF_R2_JURISDICTION=eu` setzen, Redeploy abwarten, `provision` wiederholen (docs/ROUTER_R2_JURISDICTION_BINDING.md) |
| `worker: skip`, „CF_R2_JURISDICTION is not an R2 jurisdiction" | Der Wert ist keiner von `eu`, `fedramp`, `fedramp-high`. Der Upload wird **bewusst verweigert** statt still auf den Standard-Namensraum zurückzufallen — das wäre eine unbemerkte Änderung daran, wo Nutzerdaten liegen | Wert korrigieren, oder die Variable löschen, wenn der Bucket keine Jurisdiktion hat |
