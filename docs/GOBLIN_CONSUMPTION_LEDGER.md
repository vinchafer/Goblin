...
**FOUNDER-WALK-7 (mobile walk fixes, 2026-08-20): ONE unit touches consumption, negligible and cache-preserving; the other four are zero-token. NOTE, not an M-row (no new mechanism). (U1 — editor autostart) client-only: the D1 build/publish hand-off no longer auto-navigates, it stages the same `goblin:agent-pending` payload and waits for an explicit tap (`standalone-chat.tsx` `openAgentHandoff`). No prompt/token change — whatever the agent run would have cost, it still costs exactly that, once the user actually opens Code; a message the user never taps is now a message that was never run at all (a REDUCTION in accidental agent-run starts, not tracked as a formula change since the prior rate is unmeasured). (U2 — phantom session) zero model tokens: a `heartbeat_at` timestamp write per emitted SSE frame (DB only, `agent_runs`, migration 0106 authored NOT applied) and an earlier, honest terminal frame on a verified-dead run instead of a silent multi-minute timeout. No completion, no `completion_costs` row. (U3(b) — language follows the account) **+1 line (~40 tokens) added to the chat/agent system-prompt's user-preferences block, ONLY for an account with `preferred_lang='en'`.** The 'de' default (the majority of accounts today) renders NOTHING — `renderUserContext()` stays byte-identical to before this wave for a 'de' user, so the WAVE-A/D-G prefix cache is untouched for the common case; an 'en' account's prefix simply becomes its own separately-cached variant, the same mechanism WEB_SEARCH_BLOCK/PROVISION_BLOCK already use for their booleans. Reads `users.preferred_lang` (0059, already applied/live) via `loadUserPreferences()` — no new migration for this part. Same user-allowance completion, no new billing path. The U3(a) profile-page language selector removal and the editor-UI string sweep are frontend-only, zero tokens. (U4 — profile Context + metadata-clobber fix, U5 — project edit UI) zero model tokens: a React Context refactor, one Supabase Admin metadata read-modify-write fix, and a new PATCH field + dialog. Migration 0106 (U2) is the only migration in this wave; authored, NOT applied — founder applies via Supabase SQL Editor. Evidence + full unit-by-unit report: the FOUNDER-WALK-7 PR description.**
