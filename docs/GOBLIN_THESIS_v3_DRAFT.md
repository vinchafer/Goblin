...
> **Since decided and shipped — three corrections for anyone quoting this section** (added 2026-08-13, Act-2 consistency sweep; the paragraphs above are left as written on 2026-07-11). **(1) The domain is `justgoblin.app`, not `goblin.app`** — the working name was never acquired; read the substitution throughout this file. **(2) The substrate is Cloudflare Workers FREE**, decided 2026-07-27, amending the spike's own recommendation: one platform-owned router Worker + KV routes + R2 (EU jurisdiction), **$0.00/month committed**, no Workers for Platforms and no D1. **(3) Requirement (b) — hard per-app resource caps — is met in a weaker form than the word "hard" implies.** Cloudflare's native per-tenant limits need Workers for Platforms and are not available; Goblin therefore built its own, at the router: a **10,000-requests-per-app-per-UTC-day** budget carried on the KV route record and enforced at the edge (`ops-caps.ts`, `worker-source.generated.ts`). Three honest qualifications, all stated in the code's own comments: it counts **requests only** — no CPU, no bandwidth dimension; it **undercounts under a burst**, because KV is eventually consistent with a 60-second per-colo cache, so an app can serve meaningfully more than its budget before the limit bites — a ceiling with a soft edge, not a rate limiter; and it **fails open**, serving the app when the counter cannot be read, because going dark on a KV blip is the worse outcome. Behind it sit the allowlist, the kill-switch, and the plan's own 100,000-requests/day account-wide hard stop — that last one fails the whole fleet closed rather than one app, which is precisely why the per-app budget exists. Requirement (c) is met at the data layer only (R2 `eu`); Workers and KV are globally distributed and the privacy page says so. **Do not put "hard per-app caps" in a pitch.** Ledger: `docs/GOBLIN_CONSUMPTION_LEDGER.md` §"Substrate & plan".
