...
- **Vercel** — refuses to create/touch any project except `synapse-platform` or `test-*`.
...
The shield lives in the **API** (`apps/api`). But the web app calls the backend via
`NEXT_PUBLIC_API_URL`, which in `.env.local` points at **Railway production**
(`https://goblinapi-production.up.railway.app`). So by default `pnpm dev` web → **prod API**
→ prod Supabase, and the local shield never runs.

**To actually protect `pnpm dev`, point web at the local guarded API:**
...
| Vercel | `apps/api/src/lib/vercel-guard.ts` + `services/vercel-service.ts` | `guardVercelCall()` before deploy creation. Allows `synapse-platform` / `test-*`; else `[VERCEL-GUARD] Blocked …`. |
...
- A legitimate write that scopes ownership by a column **not** in the list above will be
  **blocked**. Fix: scope by a known column, add the table to `DEV_SAFE_TABLES`, or set
