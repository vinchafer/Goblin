...
7. **Nur Testaccounts — für CC** (`vinc.hafner3@`, Fallback `…4`), nie der persönliche Account des Founders. Diese Regel bindet, **womit CC arbeitet**; sie sagt nichts darüber, welche menschliche Identität Betreiber-Vollmacht hält. Die Akt-2-Betreiber-Identität ist `vinc.hafner2@gmail.com` (Arbeitskonto des Founders, zugleich das `/admin`-Konto); sie steht auf `OPS_FOUNDER_ACCOUNTS`. **CC benutzt `…2@` nie.** Gründer-Entscheidung 2026-08-08, wörtlich in `docs/AKT2_PHASE2_5_FOUNDER_CONSOLE.md` §0.
8. **Kein neuer bezahlter Dienst, kein neuer Account, keine Live-Geld-Aktion ohne Founder-Wort.** Stattdessen exakte Setup-Schritte vorbereiten.
   *Präzisierung (Gründer-Entscheid 2026-08-13, aus AKT 2 · Phase 3 · F9):* **lesende Inferenz-Aufrufe zum Messen eines Gates sind in-session erlaubt, wenn ihre Kosten im Ledger verbucht werden** — alles, was Zustand ändert (Deploy, Storage, DNS, Geld), bleibt ausgeschlossen.
