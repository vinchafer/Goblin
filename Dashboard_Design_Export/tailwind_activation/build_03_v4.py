...
def logout_svg(sz=16):
    return (f'<svg width="{sz}" height="{sz}" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.8" stroke-linecap="round" stroke-linejoin="round">'
            '<path d="M9 21H5a2 2 0 0 1-2-2V5a2 2 0 0 1 2-2h4"/><polyline points="16 17 21 12 16 7"/><line x1="21" y1="12" x2="9" y2="12"/></svg>')

def mode_tile():
    # Mobile-only mode tile (current mode + switcher), §TASK 5. Shown closed.
    return ('<div class="goblin-mode-tile" style="position:relative;flex-shrink:0;display:none">'
            '<button style="display:flex;align-items:center;gap:6px;min-height:36px;padding:6px 10px;border-radius:9px;'
            'background:rgba(0,0,0,0.18);border:none;cursor:pointer;color:var(--bone,#F4ECD8);'
            'font-family:var(--font-dash-display),Manrope,sans-serif;font-weight:600;font-size:var(--t-caption-fs)">'
            '<svg width="15" height="15" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round">'
            '<path d="M21 15a2 2 0 0 1-2 2H7l-4 4V5a2 2 0 0 1 2-2h14a2 2 0 0 1 2 2z"/></svg><span>Chat</span>'
            '<svg width="11" height="11" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="3" stroke-linecap="round" style="opacity:.7"><polyline points="6 9 12 15 18 9"/></svg>'
            '</button></div>')
...
def header():
    ...
    for tid,label,active,disabled in [("chat","Chat",True,False),("code","Code",False,True),("preview","Preview",False,True)]:
        ...
        tabs += (f'<button role="tab" style="display:flex;align-items:center;gap:5px;padding:6px 10px;border-radius:7px;'
                 f'background:{bg};color:{col};font-weight:{600 if active else 500};font-size:var(--t-caption-fs);'
                 f'font-family:var(--font-dash-display),Manrope,sans-serif;border:none;'
                 f'box-shadow:{"0 1px 2px rgba(0,0,0,0.18)" if active else "none"};opacity:{0.5 if disabled else 1};min-height:28px">'
    ...
    return (
      ...
      'flex-shrink:0;border-bottom:1px solid rgba(247,247,236,0.10);position:relative;z-index:50">'
        ...
        f'<button aria-label="Goblin home" style="background:none;border:none;cursor:pointer;padding:6px 4px;flex-shrink:0;display:inline-flex;align-items:center">{goblin_mark(26)}</button>'
        f'{mode_tile()}'
        ...
        '<div style="position:relative;flex-shrink:0"><button aria-label="Create new" style="width:30px;height:30px;border-radius:50%;'
        ...
        f'<button aria-label="Konto-Menü" style="width:30px;height:30px;border-radius:50%;background:var(--gold-700);color:#2a1f0f;'
...
def account_menu():
    ...
    def row(label):
        return (f'<div style="display:flex;align-items:center;gap:10px;padding:10px 12px;border-radius:8px;cursor:pointer">'
                f'<span style="color:var(--ink-3);display:inline-flex">{gear_svg(18)}</span>'
    ...
    return (
      ...
      'border:1px solid var(--rule-soft);border-radius:14px;box-shadow:0 24px 60px -28px rgba(15,43,30,.45);z-index:200;padding:14px 14px 12px">'
        ...
        '<div style="margin-top:6px;border-top:1px solid var(--rule-soft);padding-top:6px">'
          ...
          f'<span style="color:var(--danger);display:inline-flex">{logout_svg(18)}</span>'
...
def proj_rows_sidebar(small=False):
    ...
    for i,(name,color,st,repo,desc,t) in enumerate(PROJECTS):
        ...
        out += (f'<div style="display:flex;align-items:center;gap:{10 if small else 8}px;padding:{pad};border-radius:{8 if small else 7}px;margin-bottom:{2 if small else 1}px;{mh}'
                f'background:{"rgba(212,169,74,0.13)" if active else "transparent"};border:1px solid transparent">'
      ...
      'display:flex;flex-direction:column;height:100%;overflow:hidden;position:relative;z-index:40;flex-shrink:0">'
        ...
        '<div style="padding:10px 12px;border-top:1px solid var(--border);flex-shrink:0">'
          ...
          'border:1px solid var(--border);cursor:pointer;min-height:40px;font-family:var(--font-sans)">'
            ...
            f'<span style="color:var(--text-faint);display:inline-flex">{gear_svg(14)}</span></button></div>'
      '</aside>')
...
PAGE_STYLE = ("<style>.gobl-proj-grid{display:grid;grid-template-columns:repeat(auto-fill,minmax(240px,1fr));gap:12px}"
              ".gobl-proj-list{display:none}.goblin-acct-menu{display:block}.goblin-mode-tile{display:none}"
              ...
              ".goblin-sidebar-desktop{display:none !important}.goblin-acct-menu{display:none !important}}"
...
def dashboard_home():
    return (
      ...
      '<div class="gobl-dash-home" style="max-width:1140px;margin:0 auto;padding:40px 32px 80px">'
        '<section class="gobl-hero" style="background:var(--ink-deep);color:var(--bone);border-radius:var(--radius-lg);'
...
<html lang="de"><head><meta charset="utf-8"><title>built_03_v4 · Dashboard Home</title>
{FONTS_TAG}
<style>
...
<h1 class="exp-title">Screen 03 — Dashboard Home (definitive rebuild v4)</h1>
<p class="exp-sub">app/dashboard/page.tsx + Header + Sidebar · v4</p>
<p class="exp-note">Real Goblin mark in header (not typed text) · compact §A5.2 header chrome · NO gold borders (hero + active rows neutral; gold only as fill/tick) · NO bottom bar — mobile mode-switching via the top mode-tile (shown in mobile frame) · LocalCloudSwitch in the account menu (open, top-right, desktop frame) · model-picker emoji replaced by clean monochrome lettermarks + the brand mark (picker closed here). Real tokens + compiled CSS inlined; nothing idealised.</p>
<div class="viewports">
...
out = os.path.join(HERE, "built_03_v4.html")
