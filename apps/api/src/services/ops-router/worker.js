...
// ── Request budget (U2.6 · spike F3, denial-of-wallet) ──────────────────────

/** The UTC day a counter belongs to. UTC, not local: the edge has no local. */
function budgetDay(now) {
  return new Date(now).toISOString().slice(0, 10);
}

/**
 * Has this app used up today's allowance?
 *
 * ── Honest about what this is ────────────────────────────────────────────────
 * COARSE, and deliberately so. KV reads are eventually consistent with a 60-second
 * minimum cache per colo, and writes propagate asynchronously. So under a burst
 * this counter UNDERCOUNTS, and an app can serve meaningfully more than its budget
 * before the limit bites — across many colos, quite a lot more. It is a ceiling
 * with a soft edge, not a rate limiter.
 *
 * That is an acceptable trade here and a bad one to hide. The purpose is to stop a
 * runaway app from eating the whole account's daily allowance over hours, not to
 * enforce a precise per-second rate. A precise counter needs a Durable Object,
 * which is not on the Free plane. The CF WAF rate-limiting rule evaluated in the
 * phase report is the second layer for the burst case.
 *
 * FAILS OPEN. If the counter cannot be read, the app is served. An app going dark
 * because KV blipped is a worse outcome than an overshoot on a budget that is
 * already approximate.
 */
async function overBudget(env, record, now, ctx) {
  const budget = Number(record.dailyBudget);
  // No budget on the record → no enforcement. Stated rather than defaulted to a
  // number the operator never chose.
  if (!Number.isFinite(budget) || budget <= 0) return false;

  const key = `budget:${record.appId}:${budgetDay(now)}`;
  let used = 0;
  try {
    const raw = await env.ROUTES.get(key, { cacheTtl: 60 });
    used = Number(raw) || 0;
  } catch {
    return false; // cannot count → serve
  }

  if (used >= budget) return true;

  // The increment does not need to block the response. Where the runtime gives us
  // waitUntil we use it; in a test (or without a ctx) we simply await.
  const bump = env.ROUTES.put(key, String(used + 1), { expirationTtl: 172800 }).catch(() => {});
...
export default {
  async fetch(request, env, ctx) {
    ...
    // Budget is checked only for an app that is otherwise going to be SERVED:
    // refusal pages are cheap and must never be the thing that exhausts an
    // allowance, and a suspended app must show its suspension rather than a 429.
    if (await overBudget(env, record, Date.now(), ctx)) {
      return refuse(request, siteUrl, 'over_budget', 429, { 'retry-after': '3600' });
    }
