...
export async function pushFiles(accessToken: string, owner: string, repo: string, files: Record<string, string>, message?: string) {
  const octokit = new Octokit({ auth: accessToken });
  ...
  const { data: newCommit } = await octokit.git.createCommit({
    owner,
    repo,
    message: message?.trim() || 'Update — generated by Goblin 🔨',
