...
import { supabaseOAuth } from './routes/supabase-oauth';
import { users } from './routes/users';
import { templates } from './routes/templates';
import { chatSessions } from './routes/chat-sessions';
import { codeSessions } from './routes/code-sessions';
...
// Version handler — registered at both /version (legacy) and /api/version so the
// path matches Vercel's /api/version and both surfaces answer identically.
const versionHandler = (c: import('hono').Context) => {
  return c.json({
...
app.route('/api/supabase', supabaseOAuth);
app.route('/api/users', users);
app.route('/api/templates', templates);
app.route('/api/chat-sessions', chatSessions);
app.route('/api/code-sessions', codeSessions);
