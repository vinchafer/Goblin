...
export function GoblinMark({ size = 32, className, style }: GoblinMarkProps) {
  return (
    ...
    >
      ...
      {/* Pupils */}
      <circle cx="13" cy="13.5" r="1" fill="var(--surface-ink-2)" />
      <circle cx="20" cy="13.5" r="1" fill="var(--surface-ink-2)" />
      {/* Small pointy ears */}
      <path d="M6.5 10 L4 7 L7 9" fill="var(--brand-green)" />
      <path d="M25.5 10 L28 7 L25 9" fill="var(--brand-green)" />
      {/* Ochre accent — small nose */}
      <circle cx="16" cy="17" r="1.5" fill="var(--brand-gold)" opacity="0.8" />
    </svg>
  );
}
