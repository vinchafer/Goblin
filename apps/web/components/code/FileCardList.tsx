...
export function classifyCard(file: SessionFile, baseFiles: Record<string, string>): FileCardInfo {
...
export function FileCardList({ files, baseFiles, onOpenReader, onOpenDiff }: Props) {
  const lang = useLang();
  const [filter, setFilter] = useState("");

  const cards = useMemo(() => sortCards(files.map((f) => classifyCard(f, baseFiles))), [files, baseFiles]);
  const shown = useMemo(() => {
