      ...
      >
        ...
        <div style={{ flex: 1, overflowY: 'auto', padding: '8px 12px', minHeight: 0 }}>
          {files.map((f) => {
            ...
            const willOverwrite = existing.has(f.path);
            return (
              <button
                key={f.path}
                type="button"
              ...
              >
                ...
                <span style={{ flex: 1, minWidth: 0 }}>
                  ...
                  <span style={{ fontSize: 11.5, color: 'var(--meta, var(--text-meta))' }}>
                    {langOf(f.path)} · {lineCount(f.content)} Zeilen
                  </span>
                </span>
                {willOverwrite
        ...
        <div style={{ display: 'flex', gap: 10, padding: '12px 20px 20px', borderTop: '1px solid var(--border-subtle, var(--div))' }}>
          ...
          >
            {busy ? 'Sende…' : <>{chosen.length === files.length ? 'Alle senden' : `Auswahl senden (${chosen.length})`} <ArrowRight size={16} weight="bold" /></>}
          </button>
        </div>
      </div>
