    ...
    <section style={{ background: 'var(--cream2)', padding: '100px 40px' }}>
      <div style={{ maxWidth: 1000, margin: '0 auto' }}>
        ...
        <div style={{
          ...
          boxShadow: '0 32px 80px rgba(0,0,0,0.12), 0 0 0 1px rgba(0,0,0,0.07)',
          ...
          <div style={{ background: '#141a12', display: 'flex', flexDirection: 'column' }}>
            ...
            <div style={{ flex: 1, padding: '14px 16px', fontFamily: 'JetBrains Mono, monospace', fontSize: 11, lineHeight: 1.8 }}>
              {status === 'idle' && (
                <span style={{ color: '#1e3a1c', fontStyle: 'italic' }}>// Tap Send to Code →</span>
              )}
              {status !== 'idle' && (
                <>
                  ...
                  {CODE_LINES.map((line, i) => (
                    <span key={i} style={{ display: 'block', background: 'rgba(201,147,58,0.07)', borderLeft: '2px solid var(--ochre)', margin: '0 -16px', padding: '1px 16px', color: '#8aaa85', animation: `fadeIn 0.3s ease ${i * 0.05}s both` }}>{line}</span>
                  ))}
                </>
              )}
            </div>
