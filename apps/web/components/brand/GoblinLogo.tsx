...
/**
 * GoblinLogo — Goblin brand mark, three animation states.
 *
 * State 1 — IDLE     : static, no animation (default — topbar, passive UI)
 * State 2 — THINKING : gentle vertical bounce (AI streaming, chat input)
 * State 3 — WORKING  : slow opacity pulse (build/deploy running, sidebar status)
 *
 * Drop in: /components/brand/GoblinLogo.tsx
 *
 * Usage:
 *   <GoblinLogo state="idle" size={28} variant="gold" showWordmark />
 *   <GoblinLogo state={isStreaming ? "thinking" : "idle"} size={20} />
 *   <GoblinLogo state={isBuilding ? "working" : "idle"} size={16} />
 */
...
const GOBLIN_COLORS: Record<GoblinLogoVariant, string> = {
  gold: "#D4A737",
  white: "#FFFFFF",
  black: "#111111",
  moss: "#2D4A2B",
...
const STYLES = `
@keyframes goblin-thinking {
  0%, 100% { transform: translateY(0px); }
  50%      { transform: translateY(-3px); }
}
@keyframes goblin-working {
  0%, 100% { opacity: 1; }
  50%      { opacity: 0.6; }
}
.goblin-logo { display: inline-flex; align-items: center; gap: 0.5em; line-height: 0; }
.goblin-logo[data-state="thinking"] svg { animation: goblin-thinking 0.6s ease-in-out infinite; }
.goblin-logo[data-state="working"]  svg { animation: goblin-working  1.8s ease-in-out infinite; }
.goblin-logo .goblin-wordmark {
...
@media (prefers-reduced-motion: reduce) {
  .goblin-logo[data-state="thinking"] svg,
  .goblin-logo[data-state="working"]  svg { animation: none; }
}
`;
...
function ensureStyles() {
  ...
  if (document.getElementById("goblin-logo-styles")) {
    stylesInjected = true;
    return;
  }
  const style = document.createElement("style");
  style.id = "goblin-logo-styles";
  ...
  return (
    ...
    >
      <svg
        ...
        aria-hidden="true"
      >
        <path fill={color} d={GOBLIN_PATH} />
      </svg>
      {showWordmark && (
        <span className="goblin-wordmark" style={{ fontSize: wordmarkSize }}>
          GOBLIN
        </span>
      )}
    </span>
