  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', height: '100%' }}>
      ...
      <div style={{
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
          <div style={{ width: 26, height: 26, borderRadius: 7, background: 'rgba(212,169,74,0.15)', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
            <span style={{ fontSize: 12, fontWeight: 700, color: '#D4A94A', fontFamily: 'Fraunces, serif' }}>G</span>
          </div>
      ...
      <div style={{ flex: 1, overflowY: 'auto', padding: '12px', display: 'flex', flexDirection: 'column', gap: 10 }}>
        {messages.map(msg => (
          <div key={msg.id} style={{ display: 'flex', justifyContent: msg.role === 'user' ? 'flex-end' : 'flex-start' }}>
            <div style={{
              ...
              background: msg.role === 'user' ? '#2D4A2B' : '#fff',
        ...
        {streaming && (
          <div style={{ display: 'flex', justifyContent: 'flex-start' }}>
            <div style={{
              ...
              fontSize: 12, lineHeight: 1.6, color: '#2A2A2A',
              fontFamily: 'DM Sans, sans-serif', whiteSpace: 'pre-wrap',
            }}>
              {streamingText || (
                <div style={{ display: 'flex', gap: 3, alignItems: 'center', height: 16 }}>
                  {[0, 1, 2].map(i => (
                    <div key={i} style={{ width: 5, height: 5, borderRadius: '50%', background: '#2D4A2B', animation: 'bounce 1.2s ease-in-out infinite', animationDelay: `${i * 0.16}s` }} />
                  ))}
                </div>
              )}
            </div>
        ...
        {rateLimited && (
          <div style={{ fontSize: 11, color: '#B85C3C', textAlign: 'center', padding: '4px 0', fontFamily: 'DM Sans, sans-serif' }}>
            {rateLimited}
          </div>
        )}
        ...
        <div style={{ display: 'flex', gap: 6, alignItems: 'center' }}>
          <input
            ...
            style={{
              ...
              outline: 'none', background: '#fff', color: '#2A2A2A',
            }}
          />
          <button
            onClick={() => send(input)}
          ...
          >
            <svg width="12" height="12" viewBox="0 0 24 24" fill="none" stroke={input.trim() && !streaming ? '#D4A94A' : '#9B9B9B'} strokeWidth="2.5" strokeLinecap="round">
